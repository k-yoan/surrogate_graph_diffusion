...
### These functions represent the mapping from the parameter y to the solution of the ED (or TED) diffusion equation.
...
def ed_diff_function(y):
  ...
  diff = ed_diffusion(M, x0, 0.01, T=1)
...
def ted_diff_function(y):
  '''
  This function takes a diffusion parameter y and returns the solution to the TED diffusion equation
  at the 2nd node (the specific node does not matter, it can be changed) and at time T.
  Here, we take the solution at the last iterated time step.
  '''
  ...
  diff = ted_diffusion(x0, 0.01, M_t, A, sizes)

  return diff[2,-1]



### These functions are used to compute the accuracy of the polynomial approximation (the metric used is the Root Mean Square Error).
...
# The following functions are used to generate the data for the convergence plot "Average RMSE vs. number of sample points".
