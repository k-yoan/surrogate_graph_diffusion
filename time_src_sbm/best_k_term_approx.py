...
def main(hparams):
	...
	if os.path.exists(f'time_coeff_ls_order{order}_{name_basis}_{nb_samples}.npy') and override:
		coeff_ls = np.load(f'time_coeff_ls_order{order}_{name_basis}_{nb_samples}.npy')
	else:
		coeff_ls = get_coefficients(nb_samples, ls_numpy, conf_vars, dim=d, basis=basis, ord=order)
		np.save(f'time_coeff_ls_order{order}_{name_basis}_{nb_samples}.npy', coeff_ls)
	...
	plt.loglog()
	plt.tight_layout()
	plt.savefig(f'time_best_s_term_{order}_{name_basis}_{nb_samples}.pdf')
	...
	plt.semilogy()
	plt.tight_layout()
	plt.savefig(f'time_coefficients_plot_unsorted_{order}_{name_basis}_{nb_samples}.pdf')
	...
	plt.loglog()
	plt.tight_layout()
	plt.savefig(f'time_coefficients_plot_sorted_{order}_{name_basis}_{nb_samples}.pdf')
