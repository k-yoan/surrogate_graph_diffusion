...
def main(hparams):
	...
	def best_k_term_approx(coeffs, k):
		...
		# argpartition is faster than sorting for large arrays: O(n) vs O(n log n)
		
		return np.linalg.norm(np.sort(np.abs(coeffs))[::-1][k:])
  		# Create a mask of zeros and fill the top k indices
		# approx_coeffs = np.zeros_like(coeffs)
		# approx_coeffs[idx] = coeffs[idx]
		# error = np.linalg.norm(coeffs - approx_coeffs)
	...
	plt.loglog()
	plt.show()
	plt.savefig('best_k_term_unsorted.pdf')
...
if __name__ == '__main__':
	...
	parser.add_argument('--order', type=int, default=25, help='Order of the multi-index set')
